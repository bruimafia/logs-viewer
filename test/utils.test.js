...
test('highlightJson: null/undefined → пустая строка', () => {
  assert.equal(highlightJson(null), '');
  assert.equal(highlightJson(undefined), '');
});

test('highlightJson: пустая строка → пустая строка', () => {
  assert.equal(highlightJson(''), '');
});

test('highlightJson: ключи и строковые значения получают разные классы', () => {
  const out = highlightJson('{"name":"alice"}');
  // ВАЖНО: highlightMatch экранирует кавычки как &quot;, поэтому в regex
  // ожидаем именно &quot;, не сырые ".
  assert.match(out, /<span class="json-key">&quot;name&quot;<\/span>/);
  assert.match(out, /<span class="json-string">&quot;alice&quot;<\/span>/);
});

test('highlightJson: число оборачивается в json-number', () => {
  const out = highlightJson('{"age":42}');
  assert.match(out, /<span class="json-number">42<\/span>/);
});

test('highlightJson: отрицательные, дробные и экспоненциальные числа', () => {
  assert.match(highlightJson('{"x":-1.5}'),   /<span class="json-number">-1\.5<\/span>/);
  assert.match(highlightJson('{"x":1e10}'),   /<span class="json-number">1e10<\/span>/);
  assert.match(highlightJson('{"x":1.5e-3}'), /<span class="json-number">1\.5e-3<\/span>/);
});

test('highlightJson: булевы и null получают свои классы', () => {
  const out = highlightJson('{"a":true,"b":false,"c":null}');
  assert.match(out, /<span class="json-boolean">true<\/span>/);
  assert.match(out, /<span class="json-boolean">false<\/span>/);
  assert.match(out, /<span class="json-null">null<\/span>/);
});

test('highlightJson: подстрока "true" внутри строки не считается литералом', () => {
  const out = highlightJson('{"x":"true"}');
  assert.match(out, /<span class="json-string">&quot;true&quot;<\/span>/);
  assert.ok(!/<span class="json-boolean">/.test(out));
});

test('highlightJson: HTML-спецсимволы внутри строк экранируются', () => {
  const out = highlightJson('{"x":"<script>"}');
  assert.ok(!out.includes('<script>'));
  assert.ok(out.includes('&lt;script&gt;'));
});

test('highlightJson: поисковая подсветка работает поверх синтаксической', () => {
  // Запрос `name` должен подсветиться <mark>'ом ВНУТРИ span'а json-key.
  const out = highlightJson('{"name":"alice"}', 'name');
  assert.match(
    out,
    /<span class="json-key">.*<mark class="search-match">name<\/mark>.*<\/span>/
  );
});

test('highlightJson: пустая поисковая строка — только синтаксическая раскраска', () => {
  const out = highlightJson('{"x":1}', '');
  assert.ok(!out.includes('<mark'));
  assert.match(out, /<span class="json-key">&quot;x&quot;<\/span>/);
  assert.match(out, /<span class="json-number">1<\/span>/);
});

test('highlightJson: поиск без совпадений — без <mark>, но с раскраской', () => {
  const out = highlightJson('{"x":42}', 'zzz');
  assert.ok(!out.includes('<mark'));
  assert.match(out, /<span class="json-key">&quot;x&quot;<\/span>/);
  assert.match(out, /<span class="json-number">42<\/span>/);
});

test('highlightJson: вложенные структуры — все уровни раскрашены', () => {
  const out = highlightJson(JSON.stringify({ a: { b: 1 } }, null, 2));
  assert.match(out, /<span class="json-key">&quot;a&quot;<\/span>/);
  assert.match(out, /<span class="json-key">&quot;b&quot;<\/span>/);
  assert.match(out, /<span class="json-number">1<\/span>/);
});

test('highlightJson: массив значений', () => {
  const out = highlightJson('[1,"two",true,null]');
  assert.match(out, /<span class="json-number">1<\/span>/);
  assert.match(out, /<span class="json-string">&quot;two&quot;<\/span>/);
  assert.match(out, /<span class="json-boolean">true<\/span>/);
  assert.match(out, /<span class="json-null">null<\/span>/);
});

test('highlightJson: некорректный JSON не падает, просто без подсветки', () => {
  const out = highlightJson('not json');
  assert.equal(out, 'not json');
});

test('highlightJson: ключ с экранированной кавычкой внутри', () => {
  // JSON-исходник: {"a\"b":1} (в JS-литерале нужно удвоить \).
  // На выходе HTML: кавычки экранируются как &quot;, бэкслеш — остаётся \.
  const out = highlightJson('{"a\\"b":1}');
  assert.match(out, /<span class="json-key">&quot;a\\&quot;b&quot;<\/span>/);
  assert.match(out, /<span class="json-number">1<\/span>/);
});

test('highlightJson: `truestory` не считается boolean (нужна граница слова)', () => {
  const out = highlightJson('{"x":"truestory"}');
  assert.match(out, /<span class="json-string">&quot;truestory&quot;<\/span>/);
  assert.ok(!/<span class="json-boolean">/.test(out));
});

test('highlightJson: переводы строк и отступы между токенами не теряются', () => {
  // JSON.stringify(obj, null, 2) расставляет переводы строк и пробелы —
  // они должны попасть в выход как есть.
  const out = highlightJson(JSON.stringify({ a: 1, b: 2 }, null, 2));
  assert.ok(out.includes('\n'));
});

test('highlightJson: разделители между токенами идут без обёртки', () => {
  // После </span> числа должна идти запятая (без обёртки), потом
  // снова <span class="json-key">.
  const out = highlightJson('{"a":42,"b":true}');
  assert.match(out, /<\/span>,<span class="json-key">/);
});
