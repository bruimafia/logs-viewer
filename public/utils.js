...
/**
 * Палитра иконок-маркеров для сервисов (пункт 6.5 плана улучшений).
 * Только монохромные геометрические юникод-глифы — рендерятся одинаково
 * на всех платформах, легко перекрашиваются под цвет сервиса через
 * `color`, читаемы при размере 11–12 px. Подбор детерминирован — индекс
 * берётся из того же хэша, что и hue цвета, поэтому одно имя сервиса
 * всегда даёт одну и ту же иконку.
 *
 * Структура — 10 форм × 2 варианта заливки (заполненный/контурный),
 * итого 20 иконок. Это даёт ~93% вероятность уникальной иконки на 5
 * сервисах и ~13 разных иконок на 20 (vs ~7 при палитре из 10) — то есть
 * приемлемое разнообразие даже для большой инсталляции с 15–20 сервисами.
 * Все глифы из Unicode-блоков Geometric Shapes (U+25xx) и Geometric
 * Shapes Extended (U+2Bxx) — поддерживаются с Unicode 5.1 (2008).
 */
export const SERVICE_ICON_PALETTE = [
  '●', '○',  // круг
  '■', '□',  // квадрат
  '◆', '◇',  // ромб
  '▲', '△',  // треугольник вверх
 ...
 * `traceIdColor`, `serviceColor` и `serviceIcon` — чтобы детерминированно
 * выводить разные значения (цвет/иконка) из одного входа.
 *
...
/**
 * Возвращает иконку (юникод-глиф) для сервиса
 * (пункт 6.5 плана улучшений).
 *
 * Детерминирован: один сервис всегда получает одну и ту же иконку.
 * Для null/undefined/'' возвращает иконку «по умолчанию» — выбранную тем
 * же алгоритмом для пустой строки, чтобы безымянные записи тоже имели
 * визуальный маркер.
 *
 * @param {string} serviceKey
 * @returns {string} один из элементов SERVICE_ICON_PALETTE
 */
export function serviceIcon(serviceKey) {
  const n = SERVICE_ICON_PALETTE.length;
  const idx = ((hashString(serviceKey) % n) + n) % n;
  return SERVICE_ICON_PALETTE[idx];
}
