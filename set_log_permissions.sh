...
# Скрипт нужен для подготовки доступа к логам сервисов, которые лежат в
# /var/www/services. Он проходит по директориям сервисов с именами вида
# "*-service", находит внутри каждой директории одноименный .log-файл и
# выдает пользователю TARGET_USER право на чтение этого файла.
#
# Основной способ выдачи прав - ACL через setfacl: так доступ получает только
# указанный пользователь, а существующие права владельца, группы и остальных
# пользователей не меняются. Если ACL недоступны, скрипт использует запасной
# вариант chmod o+r, который открывает чтение лога для всех пользователей.
#
# После выполнения скрипт выводит статистику: сколько сервисов найдено, сколько
# логов обработано и были ли ошибки.
